package songbird.lists;

public class SessionAttributeList {

    public static final String lastIntent = "LAST_INTENT";
    public static final String isIntervallCompleted = "INTERVALS_COMPLETED";
    public static final String isLaufCompleted = "LAUF_COMPLETED";
    public static final String forRepeatIntent = "REPEAT_THIS";

    public static final String statusWelcome = "Welcome_Intent";
    public static final String statusStimme = "Stimme_Intent";
    public static final String statusIntervall = "Intervall_Intent";
    public static final String statusLaufe = "Lauf_Intent";
    public static final String statusHilfe = "Hilfe_Intent";
    public static final String statusTipp = "Tipp_Intent";

}
